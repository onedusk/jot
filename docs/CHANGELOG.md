...
- **Markdown copies landing on other sources**: When the output directory was also one of several input paths (for example `-o docs` with inputs `docs` and `README.md`), the markdown copy of a document from another input was written into it. It overwrote a different source file, or was scanned as a new source by the next build, and with `output.structure: project` every rebuild nested another `docs/docs/...` level. `build` now refuses that layout before writing anything. Building in place into the only input path, or into the project root with `output.structure: project`, keeps every copy on its own source and still works
