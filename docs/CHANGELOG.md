...
- **Project-relative output paths**: New `output.structure` setting in `jot.yml`. The default, `input`, keeps page paths relative to each input path, as before. With `project`, they are relative to the project root, the directory jot runs from, so `docs/guide.md` is built as `dist/docs/guide.html` and the input paths `jot init` generates (`docs` and `README.md`) no longer collide. `build`, `export`, `toc`, and `debug` all honor it, so it also changes the search index, `toc.xml` and llms.txt links, export `path` and `source` fields, and document IDs. Input paths outside the project root and values other than `input` or `project` (including YAML lists) are rejected, and the collision error in the default layout now mentions the setting
...
- **Unstyled sites from installed binaries**: CSS and JavaScript assets were read from `web/templates/assets/` relative to the current directory, so `jot build` run anywhere other than the jot source tree produced pages with no styles, search, or syntax highlighting, and silently skipped the missing files. Assets are now embedded in the binary with `go:embed` (dotfiles such as `.DS_Store` excluded), and a failure to write one fails the build
- **Build summary**: `jot build` no longer reports creating `assets/styles.css`, a file that was never written. The page count line now reads "Rendered N documents to HTML"; it previously claimed N HTML files, which was off by one whenever an `index.html` was added
...
- **Markdown copies landing on other sources**: When the output directory was also one of several input paths (for example `-o docs` with inputs `docs` and `README.md`), the markdown copy of a document from another input was written into it. It overwrote a different source file, or was scanned as a new source by the next build, and with `output.structure: project` every rebuild nested another `docs/docs/...` level. Likewise, with input `docs` and output `.`, the copy of `docs/README.md` replaced the project's own `README.md`, which was not an input. Before writing anything, `build` now refuses an output directory that is or contains an input path unless every copy lands on its own source; jot cannot tell its own earlier copies from files the user wrote there. This also refuses layouts such as sources in `docs/src` with output `docs`, which lost nothing when no names clashed. Building in place into the only input path, or into the project root with `output.structure: project`, keeps every copy on its own source and still works
- **Pages silently overwritten across input roots**: Document paths are relative to each input root, so with the input paths `jot init` generates (`docs` and `README.md`), `docs/README.md` and `README.md` both wrote `README.html` and one page was lost while the build reported both as generated. `build` and `export` now fail with an error naming both source files. Output paths are compared case-insensitively, since `README.md` and `readme.md` are the same file on the default macOS and Windows filesystems. The check runs before `clean` empties the output directory, so a failed build leaves the previous site in place. A file reached through two overlapping input paths (for example `.` and `README.md`) is scanned once rather than reported as a collision, and so is one file reached through a symlink or a differently-cased path when both spellings map to the same page; spellings that map to different pages are built as separate pages, like any alias
- **No `index.html` for README-rooted sites**: When the root document was `README.md`, no index page was generated and the page was only written as `README.html`, so static hosts (GitHub Pages, S3, Netlify) returned 404 at `/` and the header logo link was broken. When there is no root `index.md`, the root README is now also written as `index.html`. Both names are matched case-insensitively, so a root `Index.md` is not overwritten by the README on case-insensitive filesystems and a lowercase `readme.md` is used. `jot serve` now serves `index.html` at `/`, as a static host would, instead of preferring `README.html`
...
- **Token counting needed the network**: `pkg/tokenizer` downloaded the `cl100k_base` vocabulary from OpenAI on first use, so `jot export` (JSONL, markdown, and the RAG and training presets) failed offline, in sandboxed CI, and in air-gapped environments, and the test suite was not hermetic. The vocabulary is now embedded in the binary (about 1 MB larger) and the encoding is built without touching tiktoken's process-wide loader, so other tiktoken users in the same process are unaffected. The vocabulary file is exempt from Git line-ending conversion and parsed tolerantly, so checkouts with `core.autocrlf` (the Git for Windows default) work too
- **Document IDs differed on Windows**: `pkg/scanner` hashed the relative path before converting it to forward slashes, so every document in a subdirectory got a different `ID` (and chunk IDs derived from it) on Windows than on macOS and Linux. The ID is now the hash of the forward-slash path on every platform
...
- **Paths containing `.md`**: Page paths replaced the first `.md` anywhere in a path, so a directory such as `site.mdocs/` was renamed in output and its pages had no `.html` file, and a file with an uppercase `.MD` extension had its page overwritten by its markdown copy. Only a trailing `.md` extension, in any case, is replaced now
